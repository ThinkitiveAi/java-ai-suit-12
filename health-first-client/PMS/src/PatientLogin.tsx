...
import { Visibility, VisibilityOff, LocalHospital, Person, ArrowBack } from '@mui/icons-material';
...
const PatientLogin: React.FC<{ onBackToIdentity?: () => void }> = ({ onBackToIdentity }) => {
  ...
  const validate = (field: string, value: string) => {
    let error = '';
    if (field === 'identifier') {
      if (!value) error = 'Email or phone number is required.';
      else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const loginData = {
        ...
        maskedIdentifier: form.identifier.replace(/(.{2}).*(.{2})/, '$1***$2'), // Mask middle part
        deviceFingerprint: navigator.userAgent + navigator.platform,
        normalizedIdentifier: form.identifier.toLowerCase().trim(),
        validLoginRequest: true
      };
      
      console.log('🔐 Attempting patient login with:', loginData);
      ...
      if (error.response) {
        ...
        console.log('🔍 Patient login error details:', errorData);
      ...
      >
        <Fade in timeout={700}>
          <AnimatedFormPaper elevation={8}>
            <Typography variant="h5" align="center" gutterBottom fontWeight={700} sx={{ mb: 2, color: theme.palette.secondary.main }}>
              Patient Login
            </Typography>
            <Box component="form" onSubmit={handleSubmit} sx={{ mt: 1, width: '100%' }}>
              ...
              <Box display="flex" alignItems="center" justifyContent="space-between" mt={1}>
                ...
                />
                <Link href="#" underline="hover" variant="body2" sx={{ ml: 1, color: theme.palette.secondary.main, fontWeight: 500 }}>
                  Forgot Password?
                </Link>
              </Box>
              {errors.general && (
